...
> Developer-only, no auto-install. Not a production release.
...
build/workflow-integration/com.hdrtosdr.app/
  ...
  electron/                  # entire directory, dereferenced
  prototype/                 # entire directory, dereferenced
...
- Entire `electron/` and `prototype/` directories are copied dereferenced.
- `scripts/verify-spike.sh` is included as executable regular.
- No file in the bundle is a symlink (walk with `lstat` fails if any symlink).
...
## Runtime Configuration — HDRTOSDR_PYTHON
...
- `CleanUp` is called exactly once during `before-quit`.
...
- Every drag revalidates: same owner (`webContents.id`), canonical non-symlink output root (`lstat`+`realpath` equals stored), direct containment via non-creating `isSafeOutputFile` helper, output `lstat` regular non-symlink, and current `realpath` equals stored canonical (`TOCTOU` check).
...
- **Executor / verifier generalization:** `b-executor.cjs` routes strictly by `profileId` (centralized map, no duplicate process logic; `checkCapability` still uses `spawnSync(..., {shell:false})` and for PQ requires `bt.2390`+`perceptual`+`peak_detect` and for all profiles requires `libx264`+AAC via argv/no-shell probes; capability failure → `profile_unavailable`); `b-executor.cjs` builds locked H.264 MP4 contract: `libx264` High preset medium CRF 18 `yuv420p` with explicit `bt709`/`tv`, AAC 192k `-map 0:a?`, `-movflags +faststart+write_colr`, `-fps_mode passthrough`, metadata/chapters stripped, `-n`; `conversion-service.cjs` allocates `buildDisplayName(base, profileId)` → `_sdr_rec709_h264_<profile>.mp4` (staging `_*.partial.mp4` valid for MP4 muxing) and spawns `verify-spike.sh <src> <dst> <profileId>` with `spawn(..., [src, dst, profileId], {shell:false})` (argv-safe, profile routing/capability). `scripts/verify-spike.sh` enforces: unknown profile fails; `hlg-local-b-v1` → exact SHA gate (two known fingerprints); `hlg-rec709-v1` → exact HLG input metadata check (`bt2020nc`/`arib-std-b67`/`bt2020`/`tv`/allowed `pix_fmt`/`has_dovi==false`) before accepting output; `pq-rec709-v1` → strict input re-gate using stream + first-frame side data (requires both `MDCV`+`CLLI`, rejects `DOVI`/`HDR10+`, exact `bt2020nc`/`smpte2084`/`bt2020`/`tv`/allowlist) before accepting output; all retain `source!=output`, timing/frame, Rec.709 H.264 High tags (`bt709`/`tv`/`yuv420p`/`h264`/`High`), privacy scan (`com.apple.quicktime.*`/`ISO6709`/creation-time), and atomic no-clobber commit. `outputStore.allocateUniqueFinalPath` collision loops remain bounded and `buildDisplayName(..., profileId)` includes profile suffix.
...
```bash
npm test          # python + electron IPC tests (includes generic HLG + narrow PQ: parser first-frame side data, positive PQ, missing MDCV/CLLI, 8-bit, wrong tags/range, numeric/string transfer 16, DOVI priority, HDR10+ reject, contract pairing, token/reinspection, profile routing/capability, renderer copy/format, verifier argv)
npm run check
npm run bundle:resolve
git diff --check
